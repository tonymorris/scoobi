...
def vectorByMatrix[T: WireFormat, V: WireFormat, R: WireFormat](
  ...
  zero: R,
  mult: (V, T) => R,
  add: Reduction[R]): InMemDenseVector[R] = error("") // matrixByVector(m.data, dv, zero, (a: T, b: V) => mult(b, a), add)
...
def vectorByMatrix[Elem: WireFormat: Ordering, T: WireFormat, V: WireFormat, R: WireFormat](
  ...
  zero: R,
  mult: (T, V) => R,
  add: Reduction[R]): InMemVector[Elem, R] = error("") // matrixByVector(m.data, dv, (a: V, b: T) => mult(b, a), add)
...
def matrixBySparseFunc[Elem: WireFormat: Ordering, V: WireFormat, Value: WireFormat, Q: WireFormat](
  ...
  add: Reduction[Q]): DMatrix[Elem, Q] =
  {
    ...
    /*
    [error]  found   : java.lang.Object with com.nicta.scoobi.Scoobi.BasicDoFn[(Elem, Iterable[((Elem, Elem), Value)]),((Elem, Elem), Q)]
    [error]  required: com.nicta.scoobi.core.DoFn[com.nicta.scoobi.core.Association1[Elem,((Elem, Elem), Value)],?]

     */
    left.groupByKey.parallelDo(
      new BasicDoFn[Association1[Elem,((Elem, Elem), Value)], ((Elem, Elem), Q)] {
