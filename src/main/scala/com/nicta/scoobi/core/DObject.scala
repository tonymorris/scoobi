...
trait DObject[A] {
  ...
  def toDList[B](implicit ev: A <:< Iterable[B], bwf: WireFormat[B]): DList[B] = toSingleElementDList.flatMap(x => x)
}
...
object DObject extends DObjects {
  ...
  def apply[A : WireFormat](x: A): DObject[A] = new DObjectImpl(x)
}
